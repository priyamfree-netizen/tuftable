    ...
    <div class="bg-gradient-to-b from-gray-50 to-white py-10">
        ...
        <div class="container mx-auto px-2 max-w-6xl mb-8">
            ...
            <p class="text-center text-xs text-gray-500 mt-3">
                Explore additional modules to extend your Tuf Table functionality
            </p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 items-stretch">
                    @foreach($modules as $module)
                        <a href="{{ $module['product_link'] }}" target="_blank" class="block">
                            <div
                                class="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 flex flex-col items-center p-4 relative group cursor-pointer">
                                <!-- Price Badge -->
                                <span
                                    class="absolute top-3 right-3 bg-blue-600 text-white text-xs font-semibold px-2 py-0.5 rounded-full shadow">
                                    ${{ $module['price'] }}
                                </span>
                                ...
                                @if($module['trending'])
                                    <span
                                        class="absolute top-3 left-3 bg-red-500 text-white text-xs font-semibold px-2 py-0.5 rounded-full shadow">
                                        Trending
                                    </span>
                                @endif
                                ...
                                <!-- Content -->
                                <h3 class="text-base font-bold text-gray-900 text-center mb-1">
                                    {{ str_replace('for Tabletrack', '', $module['product_name']) }}</h3>
                                ...
                                @if($module['rating_count'] > 0)
                                    <div class="flex items-center mb-2">
                                        <div class="flex items-center">
                                            @for($i = 1; $i <= 5; $i++)
                                                <svg class="w-3 h-3 {{ $i <= $module['rating'] ? 'text-yellow-400' : 'text-gray-300' }}"
