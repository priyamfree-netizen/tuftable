<div>
    ...
    <!-- Refund Modal -->
    <x-right-modal wire:model.live="showRefundModal">
        <x-slot name="title">
            {{ __("modules.refund.processRefund") }}
        </x-slot>

        <x-slot name="content">
            @if($selectedPayment)
                @livewire('forms.process-refund', ['payment' => $selectedPayment], key('refund-' . $selectedPayment->id))
            @endif
        </x-slot>

        <x-slot name="footer">
            <x-secondary-button wire:click="closeRefundModal" wire:loading.attr="disabled">
                {{ __('app.close') }}
            </x-secondary-button>
        </x-slot>
    </x-right-modal>
