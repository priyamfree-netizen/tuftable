<div class="grid grid-cols-1 gap-6 p-4 mx-4 mb-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
    <div>
        <div class="px-4 mb-4">
            <h3 class="mb-4 text-xl font-semibold dark:text-white">@lang('modules.settings.refundReasons')</h3>
            <x-button type='button' wire:click="showAddRefundReason" >@lang('app.add')</x-button>
        </div>
    ...
    <!-- Add Modal -->
    <x-right-modal wire:model.live="showAddRefundReasonModal">
        <x-slot name="title">
            {{ __("modules.settings.addRefundReason") }}
        </x-slot>

        <x-slot name="content">
            @livewire('forms.AddRefundReason')
        </x-slot>
    ...
    <!-- Edit Modal -->
    <x-right-modal wire:model.live="showEditRefundReasonModal">
        <x-slot name="title">
        {{ __("modules.settings.editRefundReason") }}
        </x-slot>

        <x-slot name="content">
            @if($reasonToEdit)
                @livewire('forms.EditRefundReason', ['refundReason' => $reasonToEdit], key($reasonToEdit->id))
            @endif
        </x-slot>
    ...
    <x-confirmation-modal wire:model.defer="showDeleteModal">
        ...
        <x-slot name="content">
            @lang('modules.settings.deleteRefundReasonWarning')
        </x-slot>
